...
function add_user(){
    ...
    green "=================================="
    cd /etc/wireguard/
    if [ ! -f "/etc/wireguard/002.conf" ]; then
        cp client.conf 002.conf
        wg genkey | tee temprikey | wg pubkey > tempubkey
        ipnum=$(grep Allowed /etc/wireguard/wg0.conf | tail -1 | awk -F '[ ./]' '{print $6}')
        newnum=$((10#${ipnum}+1))
        sed -i 's%^PrivateKey.*$%'"PrivateKey = $(cat temprikey)"'%' 002.conf
        sed -i 's%^Address.*$%'"Address = 10.77.0.$newnum\/24"'%' 002.conf
    cat >> /etc/wireguard/wg0.conf <<-EOF
[Peer]
...
EOF
        ...
        mv /etc/wireguard/002.conf $(curl -s ifconfig.me)
